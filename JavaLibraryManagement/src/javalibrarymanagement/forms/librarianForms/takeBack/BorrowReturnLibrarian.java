...
public class BorrowReturnLibrarian extends javax.swing.JFrame {
    ...
    private void btnTakeBackActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnTakeBackActionPerformed
        ...
        if(etReturnDate.getDate() != null){
            if(selectedRow != -1){
                ArrayList<Member> member = service.searchMembersWithName(tblMembers.getValueAt(selectedRow, 3).toString());
                result = service.returnBook(sdf.format(etReturnDate.getDate()), tblBorrows.getValueAt(selectedRow, 0).toString(), member.get(0).getUserID());
                if(result){
